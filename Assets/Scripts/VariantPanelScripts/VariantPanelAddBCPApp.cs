...
public class VariantPanelAddBCPApp : VariantPanel {
    ...
    public override Dictionary<string, string> GetVariantPanelData() {
        ...
        ret.Add(Constants.KEY_BCP_APP_INTERNAL_NAME, appNameInputField.text);
        ret.Add(Constants.KEY_BCP_DISPLAY_NAME, displayNameInputField.text);
        ret.Add(Constants.KEY_BNET_ID, bnetIDInputField.text);
    ...
    public override void PopulateVariantPanelData(Dictionary<string, string> savedData) {
        ...
        foreach (KeyValuePair<string, string> pair in savedData) {
            switch (pair.Key) {
                case Constants.KEY_BCP_APP_INTERNAL_NAME:
                    appNameInputField.text = pair.Value;
                    break;
                case Constants.KEY_BCP_DISPLAY_NAME:
                    displayNameInputField.text = pair.Value;
                    break;
                case Constants.KEY_BNET_ID:
                    bnetIDInputField.text = pair.Value;
                    break;
            }
        }
    }
